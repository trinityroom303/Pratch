...
#############################
class Block:
    def __init__(self, canvas, x, y, block_type, value, color, engine):
        self.canvas = canvas
...
class ScratchEngine:
    ...
    def execute_commands(self, commands, index):
        ...
        if cmd.block_type == "move":
            ...
            # Exécute le bloc suivant (s'il existe) répété "times" fois, puis passe au bloc suivant après le repeat.
            if index+1 < len(commands):
                times = cmd.value["times"]
                def repeat_block(iteration):
                    if iteration < times:
                        # Exécute uniquement le bloc suivant
                        self.execute_commands([commands[index+1]], 0)
                        self.root.after(300, lambda: repeat_block(iteration+1))
                    else:
                        self.root.after(300, lambda: self.execute_commands(commands, index+2))
                repeat_block(0)
                return
            else:
                next_index = index + 1
            ...
            # Exécute en boucle infinie le bloc immédiatement suivant
            if index+1 < len(commands):
                def forever_block():
                    self.execute_commands([commands[index+1]], 0)
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Moteur Scratch en Python")
